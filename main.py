...
class Text:
    caption = "<em>Столько: {0:,d} людей прослушали</em>.\n" \
              "<em>Дата публикации: {1}г</em>."

    description = "Привет, <b>{0}</b>🤚\n\n" \
    ...
    data_user = "{0} - ID пользователя\n" \
                "{1} - Имя пользователя\n" \
                "{2} - Фамилия или псевдоним пользователя\n" \
                "@{3} - Никнейм пользователя"

    legal = "Я создал BotMusic с идеей, что должен существовать легальный " \
    ...
    own_bot = "<em><b>Хочешь своего бота?</b></em>\n" \
              "Пиши - @mac_loves\n\n" \
              "--------------------\n" \
              "<em>Сообщение автоматически удалится</em>"
...
async def main(message: types.Message) -> None:
    ...
    try:
        ...
        if (response := audio.check()) == True:
            ...
            answer = await message.answer(f"Not good.\n" \
					  f"Ошибка: {response.args[0]}")
            await sleep(10)
        ...
        answer = await message.answer("Только авторизованным можно скачать :(")
        await sleep(10)
